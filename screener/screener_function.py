...
def cal_pvalue_main(input_type, cancer, stage, high_percent, low_percent, input_pvalue, cor_method):
    ...
    ## correction part
    df["FDR"] = multipletests(df["logrank_p_value"].values.tolist(),alpha=0.01, method= "fdr_bh")[1]
    df["Bonferroni"] = multipletests(df["logrank_p_value"].values.tolist(),alpha=0.01, method= "bonferroni")[1]
    print(df)
    if cor_method == "None":
